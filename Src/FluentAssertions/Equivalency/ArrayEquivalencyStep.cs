...
{
    public class ArrayEquivalencyStep : IEquivalencyStep
    {
        /// <summary>
        ...
        {
            if (AssertSubjectIsArray(context.Subject))
            {
        ...
        private static bool AssertSubjectIsArray(object subject)
        {
            bool conditionMet = AssertionScope.Current
                .ForCondition(!(subject is null))
                .FailWith("Cannot compare a one-dimensional array to <null>.");

            if (conditionMet)
            {
                conditionMet = AssertionScope.Current
                .ForCondition(subject is Array)
                .FailWith("Cannot compare a one-dimensional array to something else.");
            }
        ...
        {
            return ((Array)value).OfType<object>().ToArray();
        }
    }
}
